...
# Hc is standard enthalpy of combustion at 298K, i.e. HHV,  in MJ/mol (negative sign not used). Data from Wikipedia
...
print(f"NatGas gas composition")
...
def get_Hc(g):
    ...
    if g in gas_mixture_properties and 'Hc' in gas_mixture_properties[g]:
        ...
        hc = None
    # hc is in MJ/mol, so we need to divide by the molar volume at STP (0 degrees C, 1 atm) in m3
    Mw = do_mm_rules(g)/1000 # Mw now in kg/mol not g/mol
    ϱ_0 = get_density(g, 1.01325, 273.15) # in kg/m3
    molar_volume = ϱ_0 / Mw # in m3/mol
    
    if hc:
        return hc/molar_volume
    return None
